  ...
  return (
    ...
    >
      {items.map((item, i) => {
        ...
        if (item.link) {
          return (
            <a key={`${item.image}-${i}`} ref={(el) => { panelRefs.current[i] = el; }} href={item.link} {...panelProps}>
              {body}
            </a>
          );
        }
        return (
          <div key={`${item.image}-${i}`} ref={(el) => { panelRefs.current[i] = el; }} {...panelProps}>
            {body}
          </div>
        );
      })}
