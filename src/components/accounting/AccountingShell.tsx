...
export function AccountingShell({
  ...
  useEffect(() => {
    ...
    const fallback = pathname.startsWith('/admin')
      ? firstAdminHref(workspace.adminPermissions) || '/accounting/dashboard'
      : accountingMenuSections.find((section) =>
      ...
      {workspace && !workspace.isPlatformAdmin && workspace.subscriptionActive === false ? (
        <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          ...
          {workspace.storeRole === 'owner' ? (
            <Link href="/accounting/profile?tab=subscription" className="font-medium underline">
              خرید اشتراک
            </Link>
          ) : (
